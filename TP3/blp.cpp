	...
	for(int i = 0; i<45; i++){
		...
		if(test_constraints(xs) == 0){// se xi = 0 nao passar nas constraints
			...
			if(test_constraints(xs)==0){ // se xi = 1 nao passar nas constraints
				...
				else{// se o resultado de xi = 1 for pior que o atual, e com xi = 0 nao passar nas constraints é impossível fazer melhor resultado que isto.
					if(bound != -99999999)
						printf("%d\n", bound);
					else{
